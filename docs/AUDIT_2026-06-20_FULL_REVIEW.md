...
| 编号 | 严重度 | 发现 | 状态 |
|---|---|---|---|
| F1 | HIGH | 自动博客瘫痪（一致性检查假阳性）| ✅ 已修（PR #74，合入 main `f144fef`）|
| F2 | HIGH | CI 不跑测试套件 | ✅ 已修（PR #75，合入 main `45b8d77`）|
| F3 | MEDIUM | 8 个失败测试（测试质量）| ✅ 已修（PR #75，63/63 绿）|
| F4 | MEDIUM | 中文文件名违规（无 404）| ✅ 已修（本 PR，git mv 对齐 slug）|
| F5 | MEDIUM | API 缺速率限制 | ✅ 已修（PR #76，待合）|
| F6 | LOW | coffee 断更 06-14 | ⏭️ 误报已撤销（本地 main 陈旧所致）|
| F7 | LOW | 内容归属表述模糊 | ✅ 已修（本 PR，表格+memories 加项目标注）|
| F8 | LOW | Node 20 / Next 14 维护项 | ✅ Node→22 + action 版本升级（本 PR）；Next 14 升级留待后续 |
