...
## ⛔ 真实性红线（最高优先级，违反任何一条 = 文章作废）
这是个人品牌站，虚构内容会直接伤害品牌信誉。第一人称只能写**真实身份 + 真实项目经历 + 观点反思**，绝不能借第一人称编造作者亲历的具体场景。

**禁止虚构以下任何具体细节（除非上面参考素材里明确写了）：**
- 具体地点名（如某酒店、某展馆、某城市的某场活动）
- 作者出席过的具体线下事件 / 展会 / 差旅 / 会议（"我在 X 站了两天""我去了 Y"）
- 具名或匿名他人，以及**以引号呈现的他人原话**（"他说：……"）
- 未真实发生的对话、采访、调研、客户接触
- 任何参考素材中没有的数字、指标、客户数、市场规模

**如果论证需要例子但素材里没有**：用明确标注的设想，如"设想一个场景""打个比方""假设有这样一位用户"——让读者一眼看出这是演绎，**不得写成真实发生过的经历**。

**只能写的"真实经历"**：参考素材 memory 里记录过的项目事实、作者作为异乡好居主理人/国际教育从业者的真实身份与公开观点。其余一律走"观点/方法论/设想"，不落具体时空人物。

## 要求
1. 字数 {min_words}-{max_words} 字
2. 使用 Markdown 格式（二级标题 ##、三级标题 ###）
3. **只能基于上面参考素材的真实案例**，不编造数据、地点、事件、人物、原话（见真实性红线）
...
6. 第一人称叙述，口吻真诚务实——但第一人称**不等于**可以虚构亲历场景（见真实性红线）
7. 适合在公众号和个人博客同步发布
